...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get("POSTGRES_URL", "sqlite:///new.db")
...
def get_older():
    result = db.session.execute(db.select(BlogPost))
    posts = result.scalars().all()
    return render_template("older.html", all_posts=posts, current_user=current_user, year=year)
...
def contact():
    if request.method == "POST":
        ...
        return render_template("contact.html", msg_sent=True, current_user=current_user, year=year)
    return render_template("contact.html", msg_sent=False, current_user=current_user, year=year)
